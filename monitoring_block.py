...
class BlockAnalyzer:
    ...
    def _check_same_address_transfer(self, call, call_function, debug=False):
        """Check if move_stake or transfer_stake has same origin and destination address
        
        Returns:
            bool: True if origin and destination are the same (should skip), False otherwise
        """
        # Only check for move_stake and transfer_stake operations
        same_address_operations = [
            'move_stake', 'move_stake_limit',
            'transfer_stake', 'transfer_stake_limit'
        ]
        
        if call_function not in same_address_operations:
            return False
        ...
        for arg in call.get('call_args', []):
            if isinstance(arg, dict):
                ...
                # For move_stake: compare hotkeys
                if call_function.startswith('move_stake'):
                    if arg_name in ['hotkey', 'origin_hotkey', 'src_hotkey']:
                        origin_hotkey = arg_value
                    elif arg_name in ['destination_hotkey', 'dest_hotkey', 'new_hotkey']:
                        dest_hotkey = arg_value
                
                # For transfer_stake: compare coldkeys
                elif call_function.startswith('transfer_stake'):
                    if arg_name in ['coldkey', 'origin_coldkey', 'src_coldkey']:
                        origin_coldkey = arg_value
                    elif arg_name in ['destination_coldkey', 'dest_coldkey', 'new_coldkey']:
                        dest_coldkey = arg_value
        
        # Check if addresses are the same
        if call_function.startswith('move_stake'):
            if origin_hotkey and dest_hotkey and origin_hotkey == dest_hotkey:
                if debug:
                    print(f"  [DEBUG] Skipping move_stake with same hotkey: {origin_hotkey}")
                return True
        
        elif call_function.startswith('transfer_stake'):
            if origin_coldkey and dest_coldkey and origin_coldkey == dest_coldkey:
                if debug:
                    print(f"  [DEBUG] Skipping transfer_stake with same coldkey: {origin_coldkey}")
                return True
    ...
    def get_extrinsic_details(self, block, extrinsic_idx, debug=False):
        ...
        try:
            ...
            # Check if this is a same-address transfer that should be skipped
            should_skip = False
            if method:
                # Check the innermost method (after >) for nested calls
                inner_method = method.split(' > ')[-1] if ' > ' in method else method
                should_skip = self._check_same_address_transfer(call, inner_method, debug)
    ...
    def get_current_block_data(self, block_number: int):
        ...
        try:
            ...
            for event in events:
                ...
                # Event format: (hotkey, coldkey, amount)
                if isinstance(attributes, (tuple, list)) and len(attributes) >= 3:
                    ...
                    # Skip if this is a same-address move_stake or transfer_stake
                    if should_skip:
                        continue
                    ...
                    # If no netuid in extrinsic, determine based on hotkey registrations
                    if netuid is None:
